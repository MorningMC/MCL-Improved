...
public record JavaRuntime(File executable, Runtime.Version version, Platform platform) implements Comparable<JavaRuntime> {
	...
	/**
	 * Represents a collection of Java runtimes.
	 */
	@ObjectCollection
	@StaticClass
	public static class Collection implements Runnable {
		...
		public static final NbtLoader<Void, ListTag<StringTag>> loader = new NbtLoader<>() {
			...
			/**
			 * Load {@link Collection} from an NBT list tag.
			 *
			 * @param tag The NBT list tag containing Java runtime paths.
			 * @return {@code null}.
			 */
			@Override
			public Void load(ListTag<StringTag> tag) {
		...
		/**
		 * Initialize the {@link Collection} with a collection of Java runtimes.
		 *
		 * @param runtimes The collection of Java runtimes.
		 */
		public static void init(java.util.Collection<JavaRuntime> runtimes) {
			Collection.runtimes.addAll(runtimes);
		}
		...
		public static void search() {
			if (!isSearching()) {
				thread = new Thread(new Collection(), "java");
				thread.start();
			}
		}
				...
				try {
					...
					switch (Platform.current.operatingSystem()) {
						case WINDOWS -> {
							...
							Stream.of("ProgramFiles", "ProgramFiles(x86)", "ProgramFiles(ARM)")
									...
									.flatMap(Collection::listDirectories)
									.parallel()
									.flatMap(Collection::parseHome)
					...
					try {
						Arrays.stream(System.getenv("PATH").split(Platform.current.pathSeparator()))
								...
								.map(bin -> new File(bin, executableName))
								.flatMap(executable -> {
									logger.trace("Query executable in PATH: {}", executable);
									try {
										return Stream.of(fromPath(executable));
									} catch (IllegalJavaException e) {
										return Stream.empty();
									}
								})
		...
		private static Stream<JavaRuntime> parseHome(File home) {
			...
			try {
				return Stream.of(fromHome(home));
			} catch (IllegalJavaException e) {
				return Stream.empty();
			}
		}
